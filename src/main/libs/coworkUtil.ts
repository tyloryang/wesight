...
function resolveWindowsGitBashPath(): string | null {
  ...
  // 2. Bundled PortableGit (preferred default in WeSight package)
...
function ensureWindowsBashUtf8InitScript(): string | null {
  try {
    ...
    const content = [
      '#!/usr/bin/env bash',
      '# Auto-generated by WeSight – switch Windows console code page to UTF-8',
...
function removeCoworkModelEnv(env: Record<string, string | undefined>): void {
  for (const key of COWORK_MODEL_ENV_KEYS) {
    delete env[key];
  }
}
...
export async function getEnhancedEnv(
  ...
  if (!shouldInjectCoworkModelConfig) {
    removeCoworkModelEnv(env);
  }
...
export async function getEnhancedEnvWithTmpdir(
  ...
  const env = await getEnhancedEnv(target, options);
