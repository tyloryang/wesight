...
export class SqliteStore {
  ...
  private initializeTables(basePath: string) {
    ...
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS external_agent_providers (
        id TEXT NOT NULL,
    ...
    this.db.exec(`
      CREATE INDEX IF NOT EXISTS idx_external_agent_providers_app_current
      ON external_agent_providers(app_type, is_current);
    `);
