 ...
 *   setLanguage('en');
 *   const label = t('trayShowWindow'); // "Open WeSight"
...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    externalCliClaudeImageUnsupported: 'Claude Code CLI 暂不支持在该集成中直接传入图片，已继续发送文本上下文。',
    externalCliClaudeWaitingForOutput: '[Claude Code] 已加载本机配置（{provider}），正在等待 CLI 返回内容。',
    ...
    skillErrInvalidSource: '无效的技能来源。支持 owner/repo、仓库链接、npm 包名、SkillHub/ClawHub 链接或 GitHub tree/blob 链接。',
    skillErrClawhubNotFound: '在 ClawHub 上未找到该技能，请检查链接是否正确。',
    skillErrClawhubDownloadFailed: '从 ClawHub 下载技能失败，请稍后重试。',
    skillErrSkillHubDownloadFailed: '从 SkillHub 下载技能失败，请稍后重试。',
    ...
    imFeishuHermesHint: '飞书将通过 Hermes Agent 运行，WeSight 已同步当前启用实例到 Hermes 配置。',
    imFeishuClaudeCodeHint: '飞书将由 WeSight 原生长连接接入，并通过本机 Claude Code CLI 执行任务。',
    imFeishuClaudeCodeCliReady: 'Claude Code CLI 已就绪: {path}',
    imFeishuClaudeCodeCliMissing: '未检测到 Claude Code CLI。',
    imFeishuClaudeCodeCliMissingSuggestion: '请先在 Agent 引擎设置中安装 Claude Code，或确认终端可运行 claude。',
    imFeishuCodexHint: '飞书将由 WeSight 原生长连接接入，并通过本机 Codex CLI 执行任务。',
    imFeishuCodexCliReady: 'Codex CLI 已就绪: {path}',
    imFeishuCodexCliMissing: '未检测到 Codex CLI。',
    imFeishuCodexCliMissingSuggestion: '请先在 Agent 引擎设置中安装 Codex，或确认终端可运行 codex。',
    imFeishuHermesSingleInstance: 'Hermes Agent 当前只支持一个启用的飞书机器人。',
    imFeishuHermesSingleInstanceSuggestion: '请关闭多余飞书实例，只保留一个启用实例。',
    imFeishuHermesSyncFailed: 'Hermes Agent 飞书配置同步失败。',
    imFeishuHermesSyncFailedSuggestion: '请检查 Hermes Agent 是否安装完成，并确认飞书实例配置完整。',
    imFeishuLocalOpenClawOwnerSuggestion: '当前飞书配置归本机 OpenClaw 管理。请先在飞书设置中切换为 WeSight 统一管理，再启动当前引擎的飞书链路。',
    imFeishuUnsupportedEngine: '飞书机器人当前支持 OpenClaw、Hermes Agent、Claude Code 和 Codex。',
    imFeishuUnsupportedEngineSuggestion: '请在 Agent 引擎设置中切换到 OpenClaw、Hermes Agent、Claude Code 或 Codex 后再启动飞书。',
  ...
  en: {
    ...
    externalCliClaudeImageUnsupported: 'Claude Code CLI image input is not available in this integration yet. Text context was still sent.',
    externalCliClaudeWaitingForOutput: '[Claude Code] Loaded local config ({provider}) and is waiting for CLI output.',
    ...
    skillErrInvalidSource: 'Invalid skill source. Use owner/repo, repo URL, npm package spec, SkillHub or ClawHub URL, or a GitHub tree/blob URL.',
    ...
    skillErrSkillHubDownloadFailed: 'Failed to download skill from SkillHub. Please try again later.',
    ...
    imFeishuHermesHint: 'Feishu will run through Hermes Agent. WeSight has synced the enabled instance into Hermes config.',
    imFeishuClaudeCodeHint: 'Feishu will use the WeSight native long connection and execute tasks through the local Claude Code CLI.',
    imFeishuClaudeCodeCliReady: 'Claude Code CLI is ready: {path}',
    imFeishuClaudeCodeCliMissing: 'Claude Code CLI was not detected.',
    imFeishuClaudeCodeCliMissingSuggestion: 'Install Claude Code in Agent Engine settings, or make sure the claude command works in Terminal.',
    imFeishuCodexHint: 'Feishu will use the WeSight native long connection and execute tasks through the local Codex CLI.',
    imFeishuCodexCliReady: 'Codex CLI is ready: {path}',
    imFeishuCodexCliMissing: 'Codex CLI was not detected.',
    imFeishuCodexCliMissingSuggestion: 'Install Codex in Agent Engine settings, or make sure the codex command works in Terminal.',
    imFeishuHermesSingleInstance: 'Hermes Agent currently supports one enabled Feishu bot.',
    imFeishuHermesSingleInstanceSuggestion: 'Disable extra Feishu instances and keep only one enabled instance.',
    imFeishuHermesSyncFailed: 'Failed to sync Feishu config to Hermes Agent.',
    imFeishuHermesSyncFailedSuggestion: 'Check that Hermes Agent is installed and the Feishu instance config is complete.',
    imFeishuLocalOpenClawOwnerSuggestion: 'Feishu is currently managed by local OpenClaw. Switch to WeSight management in Feishu settings before starting this engine link.',
    imFeishuUnsupportedEngine: 'Feishu bots currently support OpenClaw, Hermes Agent, Claude Code, and Codex.',
    imFeishuUnsupportedEngineSuggestion: 'Switch the Agent engine to OpenClaw, Hermes Agent, Claude Code, or Codex before starting Feishu.',
